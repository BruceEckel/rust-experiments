...
pub async fn calculation(name: &str, yielding: bool) {
    println!("\nStarting '{}' with yielding {}", name, yielding);
...
pub fn run_tasks(rt: &Runtime, yielding: bool) {
    rt.block_on(async {
        let start = Instant::now();
        let task_one = tokio::spawn(calculation("one", yielding));
        let task_two = tokio::spawn(calculation("two", yielding));
